...
% Mushroom image at top as a node
\node (inputimg) 
    {\includegraphics[width=0.4\columnwidth]{Sparrige_Schüppling_(Pholiota_squarrosa).jpg}};
\node[below=0.1cm of inputimg, font=\small] (inputlabel) 
    {Input: \textit{Amanita phalloides}};

% Pipeline stages
\node[box, fill=blue!15, below=0.4cm of inputlabel] (s1) 
    {Stage 1: Vision API\\\footnotesize YOLOv26n TFLite};
...
% App screenshot at bottom as a node
\node[below=0.4cm of s4] (outputimg) 
    {\includegraphics[width=0.95\columnwidth]{Gradio_UI.png}};
\node[below=0.1cm of outputimg, font=\small] (outputlabel)
    {Output: Structured report in Gradio interface};

% Arrows
\draw[arrow] (inputlabel) -- 
    node[right]{\footnotesize image} (s1);
\draw[arrow] (s1) -- 
    node[right]{\footnotesize species and confidence} (s2);
\draw[arrow] (s2) -- 
    node[right]{\footnotesize 12-field record} (s3);
\draw[arrow] (s3) -- 
    node[right]{\footnotesize verdict and reasoning} (s4);
\draw[arrow] (s4) -- 
    node[right]{\footnotesize risk level} (outputimg);

\end{tikzpicture}
\caption{End-to-end pipeline for \textit{Amanita phalloides}}
